...
#+(and ecl unicode)
(defun -unbase-strings (event)
  (loop for i in event collecting
                       (typecase i (base-string (coerce i 'string)) (list (-unbase-strings i)) (t i))))
...
(defun send-to-emacs (event)
  ...
  (without-slime-interrupts
    (let ((c *emacs-connection*)
          (event #+(and ecl unicode)
                 (-unbase-strings event)
                 #-(and ecl unicode)
                 event))
      (etypecase c
        (multithreaded-connection
